...
- The only `catch` blocks left are deliberate: `EmployeeAuditLogger`'s best-effort audit write (a failed log entry never fails the request that already succeeded) and `CurrentSqlConnection`'s probe of the configured connection strings.
