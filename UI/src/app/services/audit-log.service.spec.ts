...
describe('AuditLogService', () => {
  ...
  const API_URL = `${environment.EmployeeManagementSystemAPI}/api/Employee/auditLog`;
  ...
  it('sends the employeeGuid as a query param', async () => {
    load('guid-1');
  ...
  it('starts a new request for a different employee', async () => {
    ...
    const req = httpMock.expectOne((r) => r.url === API_URL);
    expect(req.request.params.get('employeeGuid')).toBe('guid-2');
    req.flush({ status: 200, responseMessage: 'ok', data: [] });
    await settle();
  });
});
