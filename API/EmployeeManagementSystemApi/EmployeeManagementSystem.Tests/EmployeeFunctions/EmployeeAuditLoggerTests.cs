...
public class EmployeeAuditLoggerTests
{
    ...
    public async Task Log_PassesTheGivenArgumentsThroughToTheDbLayer()
    {
        ...
        dbUtils.Setup(d => d.LogEmployeeAudit(Guid, EmployerId, "Edited", "Updated: email", It.IsAny<CancellationToken>()))
        ...
        await auditLogger.Log(Guid, EmployerId, "Edited", "Updated: email", TestContext.Current.CancellationToken);

        dbUtils.Verify(
            d => d.LogEmployeeAudit(Guid, EmployerId, "Edited", "Updated: email", It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    {
        var dbUtils = new Mock<IDbUtils>();
        dbUtils.Setup(d => d.LogEmployeeAudit(Guid, EmployerId, "Deactivated", null, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ResponseModel<object>(200, "Success!"));
        var logger = new Mock<ILogger<EmployeeAuditLogger>>();
        var auditLogger = new EmployeeAuditLogger(dbUtils.Object, logger.Object);

        await auditLogger.Log(Guid, EmployerId, "Deactivated", cancellationToken: TestContext.Current.CancellationToken);

        dbUtils.Verify(
            d => d.LogEmployeeAudit(Guid, EmployerId, "Deactivated", null, It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    {
        var dbUtils = new Mock<IDbUtils>();
        dbUtils.Setup(d => d.LogEmployeeAudit(Guid, EmployerId, "Created", It.IsAny<string?>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new InvalidOperationException("Connection string is unreachable."));
        var logger = new Mock<ILogger<EmployeeAuditLogger>>();
        var auditLogger = new EmployeeAuditLogger(dbUtils.Object, logger.Object);

        var exception = await Record.ExceptionAsync(() => auditLogger.Log(Guid, EmployerId, "Created", cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task Log_LogsAnError_WhenTheDbLayerThrows()
    {
        ...
        await auditLogger.Log(Guid, EmployerId, "Created", cancellationToken: TestContext.Current.CancellationToken);
